...
class ConnectionController extends Controller
{
    ...
    public function store(Request $request, User $user)
    {
        ...
        abort_if(Block::where(fn ($query) => $query->where(fn ($q) => $q->where('blocker_id', $me->id)->where('blocked_id', $user->id))->orWhere(fn ($q) => $q->where('blocker_id', $user->id)->where('blocked_id', $me->id)))->exists(), 403, 'You cannot interact with this user.');
        ...
        // A notification failure must not turn a successfully-created wave into a false "Try again" error.
        try {
