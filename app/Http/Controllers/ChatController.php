...
class ChatController extends Controller {
 ...
 public function show(Request $request,Conversation $conversation){abort_unless(in_array($request->user()->id,[$conversation->user_one_id,$conversation->user_two_id],true),403);abort_unless($this->connected($conversation->user_one_id,$conversation->user_two_id)&&!$this->blocked($conversation->user_one_id,$conversation->user_two_id),403);$conversation->load(['userOne','userTwo']);Message::where('conversation_id',$conversation->id)->where('sender_id','!=',$request->user()->id)->whereNull('read_at')->update(['read_at'=>now()]);$messages=$conversation->messages()->with('sender')->oldest()->paginate(50);return view('chat.show',compact('conversation','messages'));}
 public function store(Request $request,Conversation $conversation){abort_unless(in_array($request->user()->id,[$conversation->user_one_id,$conversation->user_two_id],true),403);abort_unless($this->connected($conversation->user_one_id,$conversation->user_two_id)&&!$this->blocked($conversation->user_one_id,$conversation->user_two_id),403);$data=$request->validate(['body'=>['required','string','max:2000']]);$message=$conversation->messages()->create(['sender_id'=>$request->user()->id,'body'=>trim($data['body'])]);$conversation->update(['last_message_at'=>now()]);$message->load('sender');$recipientId=$message->sender_id===$conversation->user_one_id?$conversation->user_two_id:$conversation->user_one_id;User::find($recipientId)?->notify(new NewMessageNotification($message));broadcast(new MessageSent($message))->toOthers();return response()->json(['message'=>['id'=>$message->id,'sender_id'=>$message->sender_id,'body'=>$message->body,'created_at'=>$message->created_at->toIso8601String()]]);}
