...
class ChatController extends Controller {
 public function index(Request $request){$user=$request->user();$conversations=Conversation::with(['userOne','userTwo'])->where('user_one_id',$user->id)->orWhere('user_two_id',$user->id)->orderByDesc('last_message_at')->get();$unreadCount=$this->unreadMessageCount($user->id);return view('chat.index',compact('conversations','user','unreadCount'));}
 public function unreadCount(Request $request){return response()->json(['count'=>$this->unreadMessageCount($request->user()->id)]);}
 ...
 public function messages(Request $request,Conversation $conversation){$this->authorizeConversation($request,$conversation);$after=(int)$request->integer('after',0);$query=$conversation->messages()->with('sender')->oldest();if($after>0)$query->where('id','>',$after);$messages=$query->limit(100)->get();$messages->filter(fn($m)=>$m->sender_id!==$request->user()->id&&!$m->read_at)->each(fn($m)=>$m->update(['read_at'=>now()]));return response()->json(['messages'=>$messages->map(fn($m)=>['id'=>$m->id,'sender_id'=>$m->sender_id,'sender_name'=>$m->sender->name,'body'=>$m->body,'created_at'=>$m->created_at->toIso8601String()])->values(),'unread_count'=>$this->unreadMessageCount($request->user()->id)]);}
 ...
 private function unreadMessageCount(int $userId): int{return Message::whereNull('read_at')->where('sender_id','!=',$userId)->whereHas('conversation',fn($q)=>$q->where(fn($q)=>$q->where('user_one_id',$userId)->orWhere('user_two_id',$userId)))->count();}
