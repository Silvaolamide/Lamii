...
class ChatController extends Controller
{
    ...
    public function start(Request $request, User $user)
    {
        ...
        $me = $request->user();
        abort_unless($this->connected($me->id,$user->id), 403, 'You can only chat with accepted connections.');
    ...
    public function show(Request $request, Conversation $conversation)
    {
        ...
        abort_unless($this->connected($conversation->user_one_id,$conversation->user_two_id),403);
        $conversation->load(['userOne','userTwo']);
        Message::where('conversation_id',$conversation->id)->where('sender_id','!=',$request->user()->id)->whereNull('read_at')->update(['read_at'=>now()]);
    ...
    public function store(Request $request, Conversation $conversation)
    {
        ...
        return response()->json(['message'=>['id'=>$message->id,'sender_id'=>$message->sender_id,'body'=>$message->body,'created_at'=>$message->created_at->toIso8601String()]]);
    }
