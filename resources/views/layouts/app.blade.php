...
<head>
    ...
    <title>{{ $title ?? 'Laaamiii' }}</title>
    <script src="https://cdn.tailwindcss.com"></script>
</head>
<body class="min-h-screen bg-slate-50 text-slate-900">
<header class="border-b bg-white">
    <div class="mx-auto flex max-w-5xl items-center justify-between px-5 py-4">
        <a href="{{ route('home') }}" class="text-2xl font-black tracking-tight">Laaamiii<span class="text-indigo-600">.</span></a>
        @auth
        ...
        @else
            <div class="flex gap-4 text-sm font-semibold"><a href="{{ route('login') }}">Log in</a><a href="{{ route('register') }}">Join Laaamiii</a></div>
        @endauth
    </div>
</header>
